...
type
  TThumbProgress = procedure(const aBytesRead, aTotalBytes: Int64) of object;
  ...
  private
    fOnThumbProgress: TThumbProgress;
    fOnThumbLoaded: TBookThumbLoaded;
    fOnBookUrl: TBookUrl;
    procedure DoLoadThumbAsync(const aUrl: string);

    procedure DoReceiveDataEvent(const aSender: TObject; aContentLength, aReadCount: Int64; var aAbort: Boolean);
  public
    ...
    property OnThumbProgress: TThumbProgress read fOnThumbProgress write fOnThumbProgress;
  end;
...
begin
  // adjust field size before data is loaded
  if fdMemT_Books.FindField('thumb') is TWideStringField then
    TWideStringField(fdMemT_Books.FieldByName('thumb')).Size := 255;

  RESTRequest.Execute;
end;

procedure TdmRest.DoReceiveDataEvent(
  const aSender: TObject;
  aContentLength, aReadCount: Int64; var aAbort: Boolean);
begin
  if Assigned(FOnThumbProgress) then
    TThread.Queue(nil,
      procedure
      begin
        FOnThumbProgress(aReadCount, aContentLength);
      end);
end;
